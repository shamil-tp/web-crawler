...
const crawlOnePage = async (domain) => {
    ...
    try {
        ...
        if (newLinks.size > 0 || externalLinks.size > 0) {
            const allFoundUrls = [...Array.from(newLinks), ...Array.from(externalLinks.keys())];
            
            const backlinkOps = allFoundUrls.map(link => ({
                updateOne: {
                    filter: { url: link },
                    // $inc adds 1 to the backlink counter
                    update: { $inc: { backlinks: 1 } }, 
                    // Create a skeleton document if we haven't crawled it yet!
                    upsert: true 
                }
            }));
            // We don't await this; let it run in the background to save time!
            Site.bulkWrite(backlinkOps).catch(() => {}); 
        }
